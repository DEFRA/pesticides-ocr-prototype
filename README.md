...
The refreshed GOV.UK brand is available and turned on by default in the `pesticides-ocr-prototype`. To turn it
...
```bash
docker build --target development --no-cache --tag pesticides-ocr-prototype:development .
```

Run:

```bash
docker run -e PORT=3000 -p 3000:3000 pesticides-ocr-prototype:development
```
...
```bash
docker build --no-cache --tag pesticides-ocr-prototype .
```
...
```bash
docker run -e PASSWORD=beepBoopBeep -e PORT=3000 -p 3000:3000 pesticides-ocr-prototype
```
...
```bash
docker run -it --entrypoint /bin/ash pesticides-ocr-prototype
```
